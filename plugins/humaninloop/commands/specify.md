...
```bash
command -v hil-dag >/dev/null 2>&1 || uv run hil-dag --help >/dev/null 2>&1 || { echo "ERROR: hil-dag CLI not found. Install: uv add --dev (Python) or uv tool install (non-Python) 'humaninloop-brain @ git+https://github.com/deepeshBodh/human-in-loop.git#subdirectory=humaninloop_brain'"; exit 1; }
