...
# Output results
if $JSON_MODE; then
    printf '{"FEATURE_DIR":"%s","FEATURE_SPEC":"%s","IMPL_PLAN":"%s","TASK_MAPPING":"%s","TASKS":"%s","REQUIREMENTS":"%s","CONSTRAINTS_DECISIONS":"%s","NFRS":"%s","DATA_MODEL":"%s","QUICKSTART":"%s","CONTRACTS_DIR":"%s","BRANCH":"%s","HAS_GIT":"%s"}\n' \
        "$FEATURE_DIR" "$FEATURE_SPEC" "$IMPL_PLAN" "$TASK_MAPPING" "$TASKS" "$REQUIREMENTS" "$CONSTRAINTS_DECISIONS" "$NFRS" "$DATA_MODEL" "$QUICKSTART" "$CONTRACTS_DIR" "$CURRENT_BRANCH" "$HAS_GIT"
else
